...
function harness({
  ...
  const makeNode = (id, attributes = {}) => {
    ...
    return {
      ...
      matches(selector) {
        return selector === ':hover' ? this.hovered : true;
      },
      closest() {
        return this.isDisclosure ? this : null;
      },
    };
  };
  ...
  const locationDisclosure = locationToggleMarkup ? makeNode('location-bar-toggle', {
    ...locationToggleAttributes
  }) : null;
  ...
  panel.querySelector = (selector) => {
    ...
    if (selector.split(',').some((part) => part.trim() === '.panel-title')) return {
      textContent: 'VISUAL STYLES'
    };
  ...
  location.querySelector = (selector) => {
    ...
    if (selector.split(',').some((part) => part.trim() === '.location-toolbar-label')) return {
  ...
  const focus = (node) => {
    ...
    for (const owner of [panel, location]) {
      if (owner.contains(prior)) emit(owner, 'focusout', {
        target: prior,
        relatedTarget: node
      });
      if (owner.contains(node)) emit(owner, 'focusin', {
        target: node,
        relatedTarget: prior
      });
    }
  ...
  const methods = new Function('createHoverDisclosure', 'collapsePanelOnEscape', 'document', 'window', 'clearTimeout', 'performance', 'requestAnimationFrame',
    ...
    createHoverDisclosure, collapsePanelOnEscape, document, window, (id) => timers.delete(id), {
      now: () => now
    }, () => {},
  );
  const saves = [];
  ...
  const manager = {
    ...
    ...methods,
    _savePanelCollapsedState(...args) {
      saves.push(args);
    },
    _scheduleLeftPanelLayout() {},
    _scheduleRightPanelLayout() {},
    shareLinkManager: {
      claimRestoreLane(...args) {
        claims.push(args);
      },
      onPanelStateChange() {
        shareSyncs += 1;
  ...
  return {
    ...
    key(key = 'Enter', repeat = false) {
      return emit(disclosure, 'keydown', {
        key,
        repeat
      });
    },
    keyUp(key = ' ', repeat = false) {
      const event = emit(disclosure, 'keyup', {
        key,
        repeat
      });
      if (key === ' ' && !repeat && document.activeElement === disclosure && !event.defaultPrevented) emit(disclosure, 'click', {
        detail: 0
      });
      return event;
    },
    ...
    locationKey(key = 'Enter', repeat = false) {
      return emit(locationDisclosure, 'keydown', {
        key,
        repeat
      });
    },
    locationKeyUp(key = ' ', repeat = false) {
      const event = emit(locationDisclosure, 'keyup', {
        key,
        repeat
      });
      if (key === ' ' && !repeat && document.activeElement === locationDisclosure && !event.defaultPrevented) {
        emit(locationDisclosure, 'click', {
          detail: 0
        });
      }
      return event;
    ...
    locationEscape() {
      return emit(location, 'keydown', {
        key: 'Escape',
        target: document.activeElement
      });
    },
    click(detail = 1) {
      emit(disclosure, 'click', {
        detail
      });
    },
    drain() {
      for (let i = 0; i < 40 && timers.size; i += 1) tick();
    },
  };
}
...
test('opening the unpinned Location sibling cancels the Map Source request', () => {
  const h = harness();
  h.key();
  h.manager.setPanelCollapsed('location-bar', false);
  assert.equal(h.panel.classList.contains('collapsed'), true);
  assert.equal(h.timers.size, 0);
  h.drain();
  assert.equal(h.calls(), 0);
...
test('a pinned Map Source tray retains its own request when the sibling opens', () => {
  const h = harness();
  h.panel.classes.add('dock-pinned');
  h.key();
  h.manager.setPanelCollapsed('location-bar', false);
  h.tick();
  assert.equal(h.panel.classList.contains('collapsed'), false);
...
test('Location Enter stays immediate while Space waits for native key release', () => {
  for (const key of ['Enter', ' ']) {
    ...
    assert.equal(h.location.classList.contains('collapsed'), false);
    assert.equal(h.timers.size, 0);
    const release = key === ' ' ? h.locationKeyUp(key, true) : h.emit(h.locationDisclosure, 'keyup', {
      key
    });
...
test('Location synthesized click and Escape retain focus and its accessible name', () => {
  ...
  h.focus(h.locationDisclosure);
  h.drain();
  h.emit(h.locationDisclosure, 'click', {
    detail: 0
  });
  assert.equal(h.location.classList.contains('collapsed'), false);
  assert.equal(h.calls(), 0);
