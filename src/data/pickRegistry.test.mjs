...
test('resolvePickId: Cesium-Entity-like object → its string id', () => {
  const entity = {
    id: 'cctv-atx-cam-3-center',
    properties: {}
  };
  assert.equal(resolvePickId({
    id: entity
  }), 'cctv-atx-cam-3-center');
});
...
test('ownership: sibling layers recognize each other via String-coerced ids', () => {
  ...
  try {
    ...
    assert.equal(isOwnedByOtherLayer('flights', resolvePickId({
      id: 25544
    })), true);
