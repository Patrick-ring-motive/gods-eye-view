...
export const ANALYST_LAYERS = {
  flights: {
    ...
    text: ['callsign', 'icao24', 'originCountry', 'operator', 'routeOrigin', 'routeDestination', 'aircraftClass'],
  ...
  async function query(spec = {}) {
    ...
    if (scope.kind === 'region' && scope.name) {
      ...
      const subjectCenter = Number.isFinite(subject?.lat) && Number.isFinite(subject?.lon) ?
        ...
        null;
      const center = explicitCenter || subjectCenter || {
        lat: view.lat,
        lon: view.lon
      };
      resolvedScope = {
        center,
        km: Number(scope.km) || 100
      };
