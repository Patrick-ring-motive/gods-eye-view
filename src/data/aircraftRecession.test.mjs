...
test('applyAircraftBillboardTreatment returns its documented module singleton', () => {
  ...
  const first = applyAircraftBillboardTreatment({
    ...input,
    baseScale: 1
  });
  const second = applyAircraftBillboardTreatment({
    ...input,
    baseScale: 0.5
  });
...
test('ambient model presentation receives composed alpha without changing blend semantics', () => {
  ...
  assert.equal(applyAircraftModelTreatment({
    model,
    baseColor: color,
    alpha: 0.2,
    params
  }), 1);
  assert.deepEqual(model.color, {
    alpha: 0.2,
    rgb: 'amber'
  });
