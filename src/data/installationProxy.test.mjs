...
test('installation failure reasons disclose no raw upstream error and do not guess overload', () => {
  ...
  assert.equal(militaryInstallationFailureReason({
    name: 'AbortError'
  }), 'timeout');
  assert.equal(militaryInstallationFailureReason({
...
function entry(cachedAt, elements = [{
  type: 'node',
  id: 1
}]) {
  return {
    payload: {
      elements,
      saturated: false,
      retrievedAt: new Date(cachedAt).toISOString(),
      status: 'ready'
    },
    cachedAt,
  };
...
test('snapping never pushes a bbox off the globe', () => {
  const polar = quantizeMilitaryInstallationBox({
    south: -89.99,
    west: -179.99,
    north: 89.99,
    east: 179.99
...
test('a malformed or truncated disk entry is never served', () => {
  ...
  assert.equal(militaryInstallationDiskFresh({
    payload: {
      elements: []
    }
  }, undefined, now), false);
...
test('a real miss, write, and restart round trip serves from disk', async () => {
  ...
  try {
    ...
    const miss = await resolveMilitaryInstallationTier({
      cacheKey: key,
      memoryCache: memory,
      inFlight,
      readDisk
    ...
    const hit = await resolveMilitaryInstallationTier({
      cacheKey: key,
      memoryCache: memory,
      inFlight,
      readDisk
    ...
    memory = new Map();
    const afterRestart = await resolveMilitaryInstallationTier({
      cacheKey: key,
      memoryCache: memory,
      inFlight,
      readDisk
    ...
    const staleMemory = await resolveMilitaryInstallationTier({
      cacheKey: key,
      memoryCache: memory,
      inFlight,
      readDisk
...
test('an in-flight request is joined instead of paying for a disk read', async () => {
  ...
  try {
    ...
    const result = await resolveMilitaryInstallationTier({
      ...
      readDisk: () => {
        diskReads += 1;
        return readMilitaryInstallationDisk(key, TTL_MS, dir);
      },
    });
...
test('a torn write leaves the previous entry readable', async () => {
  ...
  try {
    ...
    assert.deepEqual(survivor.payload.elements, [{
      type: 'node',
      id: 'original'
    }]);
