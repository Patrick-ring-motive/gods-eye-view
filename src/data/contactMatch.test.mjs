...
test('an exact callsign beats an exact registration, whatever the feed order', () => {
  const asCallsign = rankContactMatch({
    query: '6606',
    hex: 'aaa111',
    callsign: '6606'
  });
  const asRegistration = rankContactMatch({
    query: '6606',
    hex: 'bbb222',
    registration: '6606'
  });
...
test('a registration matches in the form the operator says it', () => {
  ...
  assert.equal(rankContactMatch({
    query: 'GABCD',
    ...hyphenated
  }), CONTACT_MATCH_TIER.REGISTRATION_EXACT);
  assert.equal(rankContactMatch({
    query: 'g-abcd',
    ...hyphenated
  }), CONTACT_MATCH_TIER.REGISTRATION_EXACT);
...
test('a civilian N-number resolves in every form it is written or spoken', () => {
  ...
  assert.equal(rankContactMatch({
    query: 'n123',
    ...civilian
  }), CONTACT_MATCH_TIER.REGISTRATION_PREFIX);
  assert.equal(rankContactMatch({
    query: '123ab',
    ...civilian
  }), CONTACT_MATCH_TIER.REGISTRATION_SUBSTRING);
});
...
test('a same-tier tie resolves the same way every time', () => {
  ...
  const a = {
    tier: CONTACT_MATCH_TIER.REGISTRATION_PREFIX,
    id: 'bbb222'
  };
  const b = {
    tier: CONTACT_MATCH_TIER.REGISTRATION_PREFIX,
...
test('a stronger tier always beats a weaker one, whichever arrived first', () => {
  const exact = {
    tier: CONTACT_MATCH_TIER.CALLSIGN_EXACT,
    id: 'zzz999'
  };
  const prefix = {
    tier: CONTACT_MATCH_TIER.REGISTRATION_PREFIX,
...
test('a non-match never becomes the best match', () => {
  assert.equal(contactMatchWins({
    tier: CONTACT_MATCH_TIER.NONE,
    id: 'aaa111'
  }, null), false);
  assert.equal(
    contactMatchWins({
