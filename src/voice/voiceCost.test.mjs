...
test('normalizeCostLimits fills gaps from the defaults', () => {
  assert.deepEqual(normalizeCostLimits({
    warnUsd: 1
  }), {
    warnUsd: 1,
    capUsd: 5
  ...
  assert.deepEqual(normalizeCostLimits(null), {
    warnUsd: 2,
    capUsd: 5
  });
});
...
test('zero or negative thresholds mean "disabled", not "stop immediately"', () => {
  ...
  const limits = normalizeCostLimits({
    warnUsd: 0,
    capUsd: -1
  });
...
test('F3: pricing by tier alone would have under-metered an overridden session', () => {
  ...
  const byTier = createVoiceCostTracker({
    tier: 'mini'
  });
...
test('F6: serializeCostLimits encodes a disabled threshold as a sentinel', () => {
  const encoded = serializeCostLimits({
    warnUsd: 0,
    capUsd: 0
  });
...
test('F6: a raw Infinity (pre-sentinel data) is still honoured on read', () => {
  assert.equal(normalizeCostLimits({
    capUsd: Infinity
  }).capUsd, Infinity);
});
...
test('the over-estimate paths that make a floor claim wrong are real', () => {
  ...
  const unknown = createVoiceCostTracker({
    modelId: 'not-a-real-model'
  });
  const cheapest = createVoiceCostTracker({
    modelId: VOICE_MODELS.mini.id
  });
  assert.ok(
