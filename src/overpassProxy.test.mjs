...
function mirrors(byUrl) {
  ...
  const fetchImpl = async (url) => {
    ...
    return {
      ...
      headers: {
        get: () => answer.contentType || 'application/json'
      }
    };
  };
  return {
...
test('only a 2xx that is neither rate-limited nor a runtime error is data', () => {
  assert.equal(overpassPayloadIsData({
    status: 200
  }), true);
  assert.equal(overpassPayloadIsData({
    status: 204
...
function proxyHandler() {
  const plugin = createViteConfig({
    mode: 'test'
  }).plugins.find(p => p.name === 'overpass-proxy');
